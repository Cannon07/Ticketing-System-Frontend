...
mod concert_ticketing_system {
    ...
    impl ConcertTicketingSystem {
        ...
        pub fn register_user(&mut self, data_hash: String) -> Result<()> {
            ...
            Self::env().emit_event(AccountRegistered {
                account: Some(caller),
                data: data_hash,
            });
    ...
    mod tests {
        ...
        fn assert_event(event: &ink::env::test::EmittedEvent, expected_from: Option<AccountId>, expected_data: String) {
            ...
            if let Event::AccountRegistered(AccountRegistered { account, data }) = decoded_event {
                assert_eq!(account, expected_from, "encountered invalid AccountRegistered.user");
                assert_eq!(data, expected_data, "encountered invalid AccountRegistered.data");
            } else {
                panic!("encountered unexpected event kind: expected a AccountRegistered event")
            }
        }
